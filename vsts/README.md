...
To learn more about pull request analysis, see the [Incremental analysis](https://www.jetbrains.com/help/qodana/inspect-your-code.html#Incremental+analysis) chapter.
...
Here, `QODANA_TOKEN` refers to the [project token](https://www.jetbrains.com/help/qodana/cloud-projects.html#cloud-manage-projects) generated by Qodana Cloud. In the classic interface editor, check the PR Mode option.
...
| `useInstalledCli`       | Use a Qodana CLI already available on `PATH` instead of downloading one. Optional.                                                                                                                                                                                                                                                     | `false`                                 |
| `envVarsToLinter`       | Comma-separated names of environment variables whose values should be passed to the Qodana linter container. Docker mode only; ignored in native mode because the linter already inherits the process environment. Optional.                                                                                                           | `""`                                    |
| `sarifArtifactLocation` | Artifact location and name used to upload qodana.sarif. SARIF SAST Scans Tab discovers `CodeAnalysisLogs`, names containing `_sdl_analysis`, and names ending `_sdl_sources`. Optional.                                                                                                                                                | `CodeAnalysisLogs`                      |
