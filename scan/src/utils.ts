 ...
 */
function prFixesBody(jobUrl: string): string {
  return ` 🖐 Hey there!
...
export async function pushQuickFixes(): Promise<void> {
  ...
  if (github.context.payload.pull_request?.head.ref !== undefined) {
  ...
  if (getInputs().pushFixes === 'branch') {
    ...
    await exec.getExecOutput('git', ['checkout', '-b', newBranchName])
    await exec.getExecOutput('git', ['push', 'origin', newBranchName])
